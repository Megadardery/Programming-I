        ...
        for(int j = 0 ; j < SIZE ; ++j){
            for(int row = start_point ; row <= end_point ; ++row){
                for(int col = start_point ; col <= end_point ; ++col){
                    ...
                    sum_red += image[i+row][j+col][0];
                    sum_green += image[i+row][j+col][1];
                    sum_blue += image[i+row][j+col][2];

                    ++counter;
                }
            }

            blurImage[i][j][0] = (sum_red / counter);
            blurImage[i][j][1] = (sum_green / counter);
            blurImage[i][j][2] = (sum_blue / counter);
